...
class MameGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        commandArray += [ "-artpath",     f"{MAME_ARTWORK_DIR}/;/usr/bin/mame/artwork/;{MAME_BIOS / 'artwork'};{USER_DECORATIONS}" ] # first for systems ; second for overlays
    ...
    def writeBezelConfig(bezelSet: str | None, system: Emulator, rom: Path, messSys: str, gameResolution: Resolution, gunsBordersSize: str | None, gunsBordersRatio: str | None) -> None:
        if messSys == "":
            tmpZipDir = MAME_ARTWORK_DIR / rom.stem # ok, no need to zip, a folder is taken too
        else:
            tmpZipDir = MAME_ARTWORK_DIR / messSys # ok, no need to zip, a folder is taken too
        ...
        if "mamezip" in bz_infos and bz_infos["mamezip"].exists():
            if messSys == "":
                artFile = MAME_ARTWORK_DIR / f"{rom.stem}.zip"
            else:
                artFile = MAME_ARTWORK_DIR / f"{messSys}.zip"
            if artFile.exists():
                artFile.unlink()
