#!/usr/bin/env bash
## INSTALADOR DE EMULATIONSTATION-DE
...
function svg_icon(){
    ...
    cat << EOF > "${USER_HOME}"/.app-icons/svg/es-de.svg
...
function appimage_desktop_file(){
  ...
  cat << EOF > /usr/local/share/applications/es-de.desktop
[Desktop Entry]
...
StartupWMClass=EmulationStation-DE
Comment=Retrogaming
Categories=Game;
EOF
}
...
function appimage_install(){
  # Proyecto GitLab
  PROJECT="es-de/emulationstation-de"

  # Codificar el nombre del proyecto
  ENCODED_PROJECT=$(echo -n "$PROJECT" | jq -sRr @uri)

  # URL de la API
  API_URL="https://gitlab.com/api/v4/projects/$ENCODED_PROJECT/releases"

  # Buscar el primer .AppImage en la última release
  APPIMAGE_URL=$(curl -s "$API_URL" | jq -r '.[0].assets.links[] | select(.name == "ES-DE_x64.AppImage") | .url')

  # Verificar y descargar
  if [[ -n "$APPIMAGE_URL" ]]; then
    echo "Descargando desde: $APPIMAGE_URL"
    mkdir -p "$INSTALL_DIR"
    curl -L "$APPIMAGE_URL" -o "$INSTALL_DIR/emulationstation-de.AppImage"
    chmod 755 "$INSTALL_DIR/emulationstation-de.AppImage"
  else
    exit 1
  fi
  ...
  ln -s "${INSTALL_DIR}/AppRun" "/usr/local/bin/es-de"
  ln -s "${INSTALL_DIR}/AppRun" "/usr/local/bin/emulationstation"
  appimage_desktop_file

}

function desinstalar() {
  rm -f "/usr/local/bin/es-de"
  rm -f "/usr/local/bin/emulationstation"
  rm -rf "${INSTALL_DIR}"
  rm -f /usr/share/applications/es-de.desktop
  rm -f "${USER_HOME}"/.app-icons/svg/es-de.svg
}
