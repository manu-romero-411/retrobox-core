...
function desinstalar() {
    ...
    rm -r "$INSTALL_DIR" /usr/share/applications/pcsx2-emu.desktop /home/$(id -nu 1000)/.app-icons/svg/pcsx2.svg
}

function deb_download(){
    LINK="https://api.github.com/repos/PCSX2/pcsx2/releases/latest"
    DEBTEMP=$(mktemp)
    mkdir -p "$INSTALL_DIR"
    chown -R 1000:1000 /opt/pcsx2

    (curl -s $LINK |
        grep "browser_download_url.*AppImage" |
        cut -d : -f 2,3 |
        tr -d \" |
        wget -O "$DEBTEMP" -i - && mv $DEBTEMP /opt/pcsx2/pcsx2.appimage
    ) || rm -r "$INSTALL_DIR"
...
function svg_icon(){
    ...
    cat << EOF > /home/$(id -nu 1000)/.app-icons/svg/pcsx2.svg
...
function appimage_desktop_file(){
  svg_icon
  cat << EOF > /usr/share/applications/pcsx2-emu.desktop
[Desktop Entry]
Name=PCSX2
Exec=$INSTALL_DIR/pcsx2.AppImage %U
Terminal=false
Type=Application
Icon=/home/$(id -nu 1000)/.app-icons/svg/pcsx2.svg
...
function appimage_install(){
  ...
  # 4. Obtener JSON de la última release
  ...
  if [[ -z "$APPIMAGE_URL" ]]; then
    echo "No se encontró ningún AppImage en la última release."
    exit 1
  fi
  ...
  ln -s "$INSTALL_DIR/pcsx2.AppImage" "/usr/local/bin/pcsx2"
