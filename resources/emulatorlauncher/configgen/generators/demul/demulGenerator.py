...
from ... import Command
from ...retrobox_paths import BIOS, SAVES, USERDATA, mkdir_if_not_exists
...
class DemulGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        # Specific rom paths
        roms1 = Path(f"{ROMS}/hikaru")
        roms1_path_win = PureWindowsPath(roms1)
        roms2 = Path(f"{ROMS}/gaelco")
        roms2_path_win = PureWindowsPath(roms2)
        roms3 = Path(f"{ROMS}/cave3rd")
        roms3_path_win = PureWindowsPath(roms3)

        plugins = Path(f"{SAVES}/demul/demul/plugins/")
