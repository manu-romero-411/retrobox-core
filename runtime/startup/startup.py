...
"""
    try:
        ES_INI.write_text(ini_content, encoding="utf-8")
    except OSError as e:
        _logger.error("Couldn't write %s: %s", ES_INI, e)
        raise
...
# --------------------------------------------------------------------------
# main — el try/finally + el handler de SIGTERM hacen el papel del antiguo
# `trap ... EXIT` de retrobox.sh: el teardown corre tanto en salida normal
# como por Ctrl+C (KeyboardInterrupt) como por `kill`.
# --------------------------------------------------------------------------
