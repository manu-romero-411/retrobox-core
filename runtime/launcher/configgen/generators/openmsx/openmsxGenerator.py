...
class OpenmsxGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        share_dir = _OPENMSX_INST / "share"
        source_settings = _OPENMSX_CFGDIR / "settings.xml"
        settings_xml = share_dir / "settings.xml"
        settings_tcl = share_dir / "script.tcl"

        # create folder if needed
        mkdir_if_not_exists(_OPENMSX_INST)
        ...
        if not share_dir.exists():
            share_dir.mkdir()
            shutil.copytree(_OPENMSX_CFGDIR, share_dir, dirs_exist_ok=True)
