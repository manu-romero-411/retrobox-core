...
from runtime.retrobox_paths import BIOS, CMDFILES_DIR, DEFAULTS_DIR, EMULATORS, MAME_ARTWORK_DIR, MAME_SOFTWARE_DIR, ROMS, SAVES, _DECORATIONS_USER_DIR, mkdir_if_not_exists
...
def generateMAMEConfigs(playersControllers: Controllers, system: Emulator, rom: Path, guns: Guns) -> None:
    ...
    if system.config.core != 'same_cdi':
        if system.config.get_bool("sharemameart", True):
            artPath = f"{MAME_ARTWORK_DIR}/;/usr/bin/mame/artwork/;{BIOS / 'lr-mame' / 'artwork'};{BIOS / 'mame' / 'artwork'};{_DECORATIONS_USER_DIR}"
        else:
